...
/**
 * UnpinLine icon from the Remix Icon library, Map category.
 */
case class UnpinLineProps(
  size: Int = 24,
...
/**
 * UnpinLine icon component.
 *
 * @example UnpinLine <> UnpinLineProps(size = 24, color = "blue")
 */
def UnpinLine = (props: UnpinLineProps) => {
  svg(
    viewBox := "0 0 24 24",
