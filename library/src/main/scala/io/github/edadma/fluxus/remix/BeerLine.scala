...
/**
 * BeerLine icon from the Remix Icon library, Food category.
 */
case class BeerLineProps(
  size: Int = 24,
...
/**
 * BeerLine icon component.
 *
 * @example BeerLine <> BeerLineProps(size = 24, color = "blue")
 */
def BeerLine = (props: BeerLineProps) => {
  svg(
    viewBox := "0 0 24 24",
