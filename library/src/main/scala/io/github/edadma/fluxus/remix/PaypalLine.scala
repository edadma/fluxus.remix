...
/**
 * PaypalLine icon from the Remix Icon library, Logos category.
 */
case class PaypalLineProps(
  size: Int = 24,
...
/**
 * PaypalLine icon component.
 *
 * @example PaypalLine <> PaypalLineProps(size = 24, color = "blue")
 */
def PaypalLine = (props: PaypalLineProps) => {
  svg(
    viewBox := "0 0 24 24",
