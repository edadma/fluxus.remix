...
/**
 * ServiceLine icon from the Remix Icon library, Business category.
 */
case class ServiceLineProps(
  size: Int = 24,
...
/**
 * ServiceLine icon component.
 *
 * @example ServiceLine <> ServiceLineProps(size = 24, color = "blue")
 */
def ServiceLine = (props: ServiceLineProps) => {
  svg(
    viewBox := "0 0 24 24",
