...
/**
 * SwitchLine icon from the Remix Icon library, Logos category.
 */
case class SwitchLineProps(
  size: Int = 24,
...
/**
 * SwitchLine icon component.
 *
 * @example SwitchLine <> SwitchLineProps(size = 24, color = "blue")
 */
def SwitchLine = (props: SwitchLineProps) => {
  svg(
    viewBox := "0 0 24 24",
